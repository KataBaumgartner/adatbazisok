...
WHERE db > 1
RETURN nev, db
// ebben az esetben (alias használat) az oszlop nevek altgr 7 aposztrofok közé tehetőek, de enélkül is elfogadja (amíg angol betűk és egy szó)
//ilyenkor kötelező (többnyire) az új név használata onnantól hogy megadtuk
```
...
RETURN p.name
// ez a legrugalmasabb megoldás, ezzel lehetne NOT EXISTRE is szűrni (tehát nem rendező és szereplő egyszerre)
```
...
FOR (p:Person)
ON (p.name, p.born)
//ha már van akkor DROP INDEX először
// SHOW INDEXES -- listázza a meglévő indexeket
```
...
//PROFILE-nál lefut az egész EXPLAIN-nél csak a terv
// használja az előbb létrehozott INDEX-et
```
